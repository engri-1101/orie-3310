...
def simplex_tableau(lp):
    ...
    lp_vars = sorted(lp_vars, key=lambda x: int(x.to_dict()['name'][1:]))
    num_basic = len(lp.constraints)
    # checks reduced costs to find basic variables
    basic_vars = sorted(lp_vars, key=lambda x: abs(x.to_dict()['dj']))[:num_basic]
    basic_varnames = set([i.to_dict()['name'] for i in basic_vars])
    basic_vars = [k for k in lp_vars if k.to_dict()['name'] in basic_varnames]
